...
### 1.1.0 — 2026-05-22
- **Подписки** — добавление профиля по URL подписки (Add from URL...). Поддерживаемые протоколы: VLESS (включая XTLS Reality), VMess, Shadowsocks, Trojan, Hysteria2. Форматы ответа: base64, plain text, готовый sing-box JSON
- **Обновление подписки** — кнопка Update в настройках профиля перескачивает конфиг с сохранённого URL
- **HWID** — при запросе подписки передаются заголовки `x-hwid`, `x-device-os`, `x-ver-os`, `x-device-model` для идентификации устройства в панели (совместимо с Remnawave)
- **Генерируемый конфиг** — формат соответствует современному API sing-box: `address` вместо `inet4_address`, `stack: mixed`, sniff, `hijack-dns`, `ip_is_private`, `domain_resolver`

### 1.0.1 — 2026-04-25
- Исправлен баг с QUIC в режиме TUN+SOCKS5
