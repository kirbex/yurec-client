...
- **Clear Now** — немедленно очищает файл. Работает и пока sing-box запущен: файл удаляется и создаётся новый пустой, sing-box продолжает писать в старый inode через открытый дескриптор — новый файл остаётся чистым до следующего запуска
- **Limit log file size** — включает автоматическую очистку при превышении лимита
- **Max size** — порог в мегабайтах (по умолчанию 10 MB). Проверяется в начале каждого нового сеанса sing-box: если файл превысил лимит, он очищается перед стартом

Техническая деталь: очистка выполняется через `removeItem` + `createFile`, а не через `truncateFile`. Это важно потому, что при `truncateFile` запущенный sing-box сразу записывает по старому смещению и восстанавливает размер файла. `removeItem` создаёт новый inode — процесс пишет в unlinked файл, который недоступен по пути на диске.
